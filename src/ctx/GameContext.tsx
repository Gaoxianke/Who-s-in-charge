...
import { evaluateStruggle, applyFactionWin, applyFactionLossPenalty, expireContest, applyDemotion } from '@/lib/promotionFaction';
...
export function GameProvider({ children }: { children: React.ReactNode }) {
  ...
  const computeDailyDelta = useCallback((current: PlayerSave) => {
    ...
    const meritDelta = 2; // P2：日均理政 +2（政绩池，与派系贡献池相互独立）
  ...
  const advanceTime = useCallback(async () => {
    ...
    // 排行榜日榜增量快照（推进前）
    const lbBefore = {
      merit: current.meritPoints ?? 0,
      contribution: current.factionContribution ?? 0,
      rank: current.rankLevel ?? 1,
    };
    ...
    let bonusMerit = 0;
    // P4：年度考核/大事件结算奖励（优秀 +100、良好 +60、合格 +30）
    const ASSESSMENT_MERIT_BONUS: Record<PlayerSave['assessmentGrade'], number> = { '特等': 120, '优秀': 100, '良好': 60, '合格': 30, '不合格': 0 };
    bonusMerit += ASSESSMENT_MERIT_BONUS[assessmentGrade] ?? 0;
    let bonusGdp = cumGdp;
    ...
    // ── R3/R4 每次推进：结算到期派遣 + 忠诚衰减 + 门客衰减 ──
    const advanceFeedback: string[] = [];
    if (updated) {
      const settled = await settleDispatches(updated.id, newGameDays);
      for (const r of settled) {
        if (r.success) {
          const parts: string[] = [];
          if (r.merit) parts.push(`政绩+${r.merit}`);
          if (r.silver) parts.push(`个人资金+${r.silver}`);
          if (r.contribution) parts.push(`派系贡献+${r.contribution}`);
          if (r.influence) parts.push(`声望+${r.influence}`);
          if (r.security) parts.push(`治安+${r.security}`);
          advanceFeedback.push(`✅ ${r.subName}「${r.type}」归来：${parts.join(' / ')}`);
        } else {
          advanceFeedback.push(`❌ ${r.subName}「${r.type}」任务失败，忠诚-5`);
        }
      }
      const decay = await dailyLoyaltyDecay(updated.id, daysToAdvance);
      if (decay.quitIds.length > 0) advanceFeedback.push(`💔 ${decay.quitIds.length} 名下属因忠诚过低离职`);
      if (decay.betrayalIds.length > 0) advanceFeedback.push(`🗡️ ${decay.betrayalIds.length} 名下属触发反水事件，请前往事件待办处理`);
      const retDecay = await dailyRetainerDecay(updated.id, daysToAdvance);
      if (retDecay.lostIds.length > 0) advanceFeedback.push(`💔 ${retDecay.lostIds.length} 名门客因忠诚过低流失`);
      if (settled.length > 0) {
        const fresh = await getSaveById(updated.id);
        if (fresh) setSave(fresh);
      }
      if (advanceFeedback.length > 0) setMeetingTaskFeedback(advanceFeedback.join('　'));
      // ── 排行榜日榜增量上报（含派遣结算后的最终值）──
      const lbLatest = await getSaveById(updated.id);
      if (lbLatest) {
        const meritGain = Math.round((lbLatest.meritPoints ?? 0) - lbBefore.merit);
        const contributionGain = Math.round((lbLatest.factionContribution ?? 0) - lbBefore.contribution);
        const rankGain = (lbLatest.rankLevel ?? 1) - lbBefore.rank;
        await Promise.all([
          recordDailyGain(updated.id, 'merit', meritGain),
          recordDailyGain(updated.id, 'contribution', contributionGain),
          recordDailyGain(updated.id, 'rank', rankGain * 100),
        ]);
      }
    }
    ...
    if (newMonth > prevMonth && updated) {
      ...
      // ══ P4 常态对抗（每 120 游戏天一次议案/资源局） ══════════════════
      // 胜方派系全员获得资源/声望加成，绝不直接给官职。
      if (newGameDays - (latestSave.lastConfrontationDay ?? 0) >= 120) {
        const fid = latestSave.primaryFaction as import('@/types/game').FactionId | '';
        const influence = latestSave.factionInfluence ?? 0;
        // 玩家派系胜率随影响力提升（40%~80%）
        const winChance = fid ? Math.min(0.8, 0.4 + influence / 250) : 0.3;
        const factionWin = Math.random() < winChance;
        if (factionWin) {
          await updateSave(latestSave.id, {
            lastConfrontationDay: newGameDays,
            meritPoints: Math.round((latestSave.meritPoints ?? 0) + 30),
            factionInfluence: Math.min(100, influence + 6),
            bossFavor: Math.min(100, (latestSave.bossFavor ?? 50) + 5),
            taxRevenue: Math.round((latestSave.taxRevenue ?? 0) + 200),
          });
        } else {
          await updateSave(latestSave.id, {
            lastConfrontationDay: newGameDays,
            factionInfluence: Math.max(0, influence - 4),
          });
        }
      }

      // ══ P4 公共事件（巡查 / 弹劾 / 舆情，影响晋升或降职） ═══════════
      if (newGameDays % 30 === 0 && Math.random() < 0.35) {
        const roll = Math.random();
        if (roll < 0.4) {
          // 巡查：政绩 -10~30、上级认可 -5~15
          await updateSave(latestSave.id, {
            meritPoints: Math.max(0, Math.round((latestSave.meritPoints ?? 0) - (10 + Math.floor(Math.random() * 21)))),
            bossFavor: Math.max(0, (latestSave.bossFavor ?? 50) - (5 + Math.floor(Math.random() * 11))),
          });
        } else if (roll < 0.75) {
          // 舆情：民心 ±10~30、政绩 ±10~30
          const good = Math.random() < 0.5;
          await updateSave(latestSave.id, {
            popularSupport: Math.max(0, Math.min(100, (latestSave.popularSupport ?? 50) + (good ? 1 : -1) * (10 + Math.floor(Math.random() * 21)))),
            meritPoints: Math.max(0, Math.round((latestSave.meritPoints ?? 0) + (good ? 1 : -1) * (10 + Math.floor(Math.random() * 21)))),
          });
        } else {
          // 弹劾：严重违规（已立案或廉洁极低）则降 1 级，否则重罚政绩
          if (latestSave.isFlagged || (latestSave.moralValue ?? 50) < 12) {
            await updateSave(latestSave.id, applyDemotion(latestSave, 1, newGameDays) as Partial<PlayerSave>);
          } else {
            await updateSave(latestSave.id, {
              meritPoints: Math.max(0, Math.round((latestSave.meritPoints ?? 0) - 40)),
              bossFavor: Math.max(0, (latestSave.bossFavor ?? 50) - 15),
            });
          }
        }
      }
      ...
      // ══ P5 任期考核（每 365 天一次） ════════════════════════════════
      // 不合格降 1 级（冷却 1825 天）；严重违规降 2 级（冷却 3650 天）。
      if (newGameDays - (latestSave.lastTenureAssessDay ?? 0) >= 365) {
        const pct = latestSave.annualRankPct ?? 50;
        const serious = latestSave.isFlagged || (latestSave.moralValue ?? 50) < 10;
        if (serious) {
          await updateSave(latestSave.id, applyDemotion(latestSave, 2, newGameDays) as Partial<PlayerSave>);
        } else if (pct < 50 || (latestSave.meritPoints ?? 0) < 100) {
          await updateSave(latestSave.id, applyDemotion(latestSave, 1, newGameDays) as Partial<PlayerSave>);
        } else {
          await updateSave(latestSave.id, { lastTenureAssessDay: newGameDays });
        }
      }
    }
