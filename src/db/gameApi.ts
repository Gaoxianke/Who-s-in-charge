...
import { RANK_CONFIG, DEFAULT_CITY_BY_LEVEL, SUB_TRAITS, DEPT_CONFIG, getDeptHeadTitle, getDeptDeputyTitle, getDeptNameByRank, getDeptPositionSubLevel, getDeptStaffRange, getZhongXuanDiaoStartRank, BAND_POSITIONS, GOVT_POSITIONS, NDA_POSITIONS, NPC_AGE_RANGE, RETIREMENT_AGE_MAP, PARTY_SCHOOL_CONFIG, NATIONAL_POLICY_POOL, randBirthPlace, pickUniversityName, npcSchoolTier, npcDegreeLabel, PROVINCE_LIST, PROVINCE_CITY_MAP, randRealStartTown, randRealStartTownByProvCity, RANK_INITIAL_FUND, RANK_MONTHLY_HEALTH_REGEN, RANK_DAILY_ENERGY_BONUS, ASSET_HEALTH_BONUS, ALL_FACTIONS } from '@/types/game';
...
function rowToPlayerSave(row: Record<string, unknown>): PlayerSave {
  ...
  // ── P1 晋升玩法修复字段读取 ──
  factionContribution: (row.faction_contribution as number) ?? 0,
  positionOccupancy: (row.position_occupancy as Record<string, number>) ?? {},
  breakPromotionCycleId: (row.break_promotion_cycle_id as number) ?? 0,
  // ── P2–P7 晋升玩法扩展字段读取 ──
  factionPosition: (row.faction_position as string) ?? null,
  factionPositionOccupancy: (row.faction_position_occupancy as Record<string, number>) ?? {},
  lastConfrontationDay: (row.last_confrontation_day as number) ?? 0,
  lastTenureAssessDay: (row.last_tenure_assess_day as number) ?? 0,
  demotionCooldownUntil: (row.demotion_cooldown_until as number) ?? 0,
  lastTransferDay: (row.last_transfer_day as number) ?? 0,
    ...
    silver: Number(row.silver as number) ?? 0,
    connections: Number(row.connections as number) ?? 0,
...
function rowToSubordinate(row: Record<string, unknown>): Subordinate {
  ...
  return {
    ...
    // ── R3 派遣 & R4 师徒 ──
    isDispatched: (row.is_dispatched as boolean) ?? false,
    dispatchType: (row.dispatch_type as import('@/types/game').DispatchType) ?? null,
    dispatchStartDay: (row.dispatch_start_day as number) ?? null,
    dispatchEndDay: (row.dispatch_end_day as number) ?? null,
...
export async function createSave(): Promise<PlayerSave | null> {
  // 使用本地会话（getSession）而非网络请求（getUser），避免 token 过期/网络异常时返回 null 导致建档失败
  const { data: sessionData } = await supabase.auth.getSession();
  const uid = sessionData.session?.user?.id;
  if (!uid) return null;

  // 幂等化：若该用户已有未完成建档的占位档，直接复用，避免重试时重复建档
  // （瞬时失败后用户重试是常见路径，必须保证 createSave 可安全重复调用）
  const { data: existing } = await supabase
    .from('player_saves')
    .select('*')
    .eq('user_id', uid)
    .eq('needs_character_creation', true)
    .order('created_at', { ascending: false })
    .limit(1)
    .maybeSingle();
  if (existing) {
    return rowToPlayerSave(existing as Record<string, unknown>);
  }
  ...
  const { data: newSave, error: createError } = await supabase
    ...
    .insert({
      ...
      contest_mode: 'direct',
      last_promotion_cycle_id: -1, // P1：从未晋升（-1），解除开局周期死锁
      // v6 扩展玩法初始值
  ...
  if (createError || !newSave) {
    if (createError) console.error('[createSave] 存档写入失败:', createError.message);
    return null;
  }
  // 存档创建成功，清除已使用的大区选择
  ...
  await initSubordinates(newSave.id, uid, (newSave as Record<string, unknown>).rank_level as number ?? 1);
  // 初始化上司任务
  await initBossTasks(newSave.id, uid);
  // 初始化案件
  await initPoliceCases(newSave.id, uid, 0);
  // 初始化城市财政（记录初始值，月度结算以 player_saves.fund_balance 为准）
  await supabase.from('city_finance').insert({
    save_id: newSave.id, user_id: uid,
...
export async function completeCharacterCreation(saveId: string, opts: {
  ...
  // 完成建档的 UPDATE 需要抵御瞬时故障：
  //  - 多标签页并发导致的 token 刷新竞争（401/Invalid Refresh Token）
  //  - PostgREST 瞬时 5xx / 网络抖动
  // 策略：最多 3 次尝试，失败间隔退避（300ms/800ms），每次失败先刷新会话再重试
  // maybeSingle 避免 0 行时抛错，保证错误信息可被记录与区分
  ...
  for (let attempt = 1; attempt <= 3; attempt++) {
    ...
    lastError = error ? { message: error.message, code: (error as { code?: string }).code } : { message: '未匹配到存档行' };
    console.error(`[completeCharacterCreation] 第 ${attempt} 次尝试失败:`, lastError.message, lastError.code ?? '');
    if (attempt < 3) {
      // 刷新会话获取新 token，并按退避间隔等待
      await supabase.auth.refreshSession().catch(() => undefined);
      await new Promise((r) => setTimeout(r, attempt === 1 ? 300 : 800));
    }
  }
  if (!data) {
    console.error('[completeCharacterCreation] 角色信息更新失败（已重试 3 次）:', lastError?.message ?? '未知错误');
    return null;
  }
  return rowToPlayerSave(data);
}
...
export async function updateSave(saveId: string, updates: Partial<{
  ...
  // ── P1 晋升玩法修复字段写入映射 ──
  if (updates.factionContribution !== undefined) dbUpdates.faction_contribution = Math.round(updates.factionContribution);
  if (updates.positionOccupancy !== undefined) dbUpdates.position_occupancy = updates.positionOccupancy;
  if (updates.breakPromotionCycleId !== undefined) dbUpdates.break_promotion_cycle_id = Math.round(updates.breakPromotionCycleId);
  // ── P2–P7 晋升玩法扩展字段写入映射 ──
  if (updates.factionPosition !== undefined) dbUpdates.faction_position = updates.factionPosition;
  if (updates.factionPositionOccupancy !== undefined) dbUpdates.faction_position_occupancy = updates.factionPositionOccupancy;
  if (updates.lastConfrontationDay !== undefined) dbUpdates.last_confrontation_day = Math.round(updates.lastConfrontationDay);
  if (updates.lastTenureAssessDay !== undefined) dbUpdates.last_tenure_assess_day = Math.round(updates.lastTenureAssessDay);
  if (updates.demotionCooldownUntil !== undefined) dbUpdates.demotion_cooldown_until = Math.round(updates.demotionCooldownUntil);
  if (updates.lastTransferDay !== undefined) dbUpdates.last_transfer_day = Math.round(updates.lastTransferDay);
  ...
  if (updates.silver !== undefined) dbUpdates.silver = updates.silver;
...
// 14个职能单位（R1.5：部门调任/任命与自动编组均使用）
const FUNCTIONAL_DEPT_KEYS: DeptKey[] = [
  'police', 'ndrc', 'finance', 'urban', 'education', 'health',
  'ecology', 'market', 'agriculture', 'personnel', 'invest', 'tax', 'petition', 'organization',
];

// 下属名册编制总数上限（R1.5）
export const SUBORDINATE_QUOTA_MAX = 145;

// 构建一名已任命下属（含完整仕途档案：姓名/院校/籍贯/特质随机）
function buildAppointedSub(
  saveId: string, userId: string,
  deptKey: DeptKey, deptPosition: 'head' | 'deputy', rankLevel: number,
) {
  const isMale = Math.random() > 0.4;
  const gender = isMale ? '男' : '女';
  const name = pickNpcName();
  const trait = SUB_TRAITS[Math.floor(Math.random() * SUB_TRAITS.length)];
  const roleLabel = deptPosition === 'head'
    ? getDeptHeadTitle(deptKey, rankLevel)
    : getDeptDeputyTitle(deptKey, rankLevel);
  const subLevel = getDeptPositionSubLevel(deptKey, rankLevel, deptPosition);
  const faction = (['reform', 'pragmatic', 'cyl', 'techno', 'local'] as const)[Math.floor(Math.random() * 5)];
  const isHead = deptPosition === 'head';
  // 正职 ability≥45，副职 ability≥35（沿用现有任命资格阈值）
  const ability = isHead ? 45 + Math.floor(Math.random() * 26) : 35 + Math.floor(Math.random() * 26);
  const loyalty = isHead ? 30 + Math.floor(Math.random() * 41) : 25 + Math.floor(Math.random() * 41);
  const integrity = isHead ? 40 + Math.floor(Math.random() * 41) : 35 + Math.floor(Math.random() * 41);
  const experience = isHead ? 5 + Math.floor(Math.random() * 11) : 3 + Math.floor(Math.random() * 8);
  const age = isHead ? 40 + Math.floor(Math.random() * 12) : 30 + Math.floor(Math.random() * 12);
  const birthYear = 2026 - age;
  const { province: bp, city: bc } = randBirthPlace();
  const tier = npcSchoolTier(Math.max(1, rankLevel - 1));
  const uniName = pickUniversityName(tier, bp);
  const degreeLabel = npcDegreeLabel(tier, Math.max(1, rankLevel - 1));
  const studyYears = degreeLabel === '博士' ? 9 : degreeLabel === '硕士' ? 6 : 4;
  const gradYear = birthYear + 18 + studyYears;
  const specialties = ['economy', 'social', 'legal', 'agriculture', 'tech', 'party', 'finance', 'military'];
  ...
  return {
    ...
    gender,
    ability, loyalty, integrity, experience,
    faction,
    sub_level: subLevel,
    is_appointed: true,
    ...
    satisfaction: 55 + Math.floor(Math.random() * 26),
    ...
    career_history: _genNpcCareerHistory(Math.max(1, rankLevel - 1), age, undefined, undefined),
    is_dispatched: false,
    dispatch_type: null,
    dispatch_start_day: null,
    dispatch_end_day: null,
    mentor_id: null,
  };
}

// 生成 42 名下属（14 职能单位 × 1 正职 + 2 副职），直接写入名册并计入编制
function buildInitialSubordinates(saveId: string, userId: string, rankLevel: number) {
  const subs: ReturnType<typeof buildAppointedSub>[] = [];
  for (const dk of FUNCTIONAL_DEPT_KEYS) {
    subs.push(buildAppointedSub(saveId, userId, dk, 'head', rankLevel));
    subs.push(buildAppointedSub(saveId, userId, dk, 'deputy', rankLevel));
    subs.push(buildAppointedSub(saveId, userId, dk, 'deputy', rankLevel));
  }
  return subs;
}

// 新档初始化：生成 42 名下属（14 单位 × 3）
export async function initSubordinates(saveId: string, userId: string, rankLevel = 3) {
  await ensureNpcNamePoolLoaded();
  const subs = buildInitialSubordinates(saveId, userId, rankLevel);
  await supabase.from('subordinates').insert(subs);
}

// 职级升级时补充生成 42 名下属（受 145 编制上限约束，已满则跳过）
export async function generateRankUpSubordinates(saveId: string, userId: string, rankLevel: number): Promise<number> {
  await ensureNpcNamePoolLoaded();
  ...
  if (current >= SUBORDINATE_QUOTA_MAX) return 0;
  const subs = buildInitialSubordinates(saveId, userId, rankLevel);
  await supabase.from('subordinates').insert(subs);
  return subs.length;
}
...
export async function transferSubordinate(
  subId: string,
  targetDept: DeptKey,
  targetPosition: 'head' | 'deputy' | 'staff',
  targetCity: string,
  rankLevel = 3,
): Promise<boolean> {
  const isStaff = targetPosition === 'staff';
  const roleLabel = isStaff
    ? `${getDeptNameByRank(targetDept, rankLevel)}科员`
    : targetPosition === 'head'
      ? getDeptHeadTitle(targetDept, rankLevel)
      : getDeptDeputyTitle(targetDept, rankLevel);
  const subLevel = getDeptPositionSubLevel(targetDept, rankLevel, isStaff ? 'deputy' : targetPosition);
  const { error } = await supabase
    .from('subordinates')
    .update({
      appointed_dept: targetDept,
      dept_position: targetPosition,
      appointed_role: roleLabel,
      position: roleLabel,
      sub_level: subLevel,
      transferred_city: targetCity.trim() || null,
      is_appointed: true,
    })
    .eq('id', subId);
  return !error;
...
function rowToRecruit(row: Record<string, unknown>): RecruitCandidate {
  return {
    ...
    faction: (row.faction as Faction) ?? 'neutral',
  };
}
...
/**
 * 手动甄选录用：玩家从候选人名册中勾选录用。
 * - 同派系（与玩家主派一致）忠诚度初始 +10
 * - 录用者统一定科员级(1)，无选调生通道
 * - 录用后自动分配到编制最空缺部门
 */
export async function confirmManualRecruits(
  saveId: string, userId: string, recruitKey: number, selectedIds: string[], playerFaction: string
): Promise<{ count: number; assignments: { name: string; dept: string; position: string }[]; recruited: RecruitCandidate[] }> {
  if (selectedIds.length === 0) return { count: 0, assignments: [], recruited: [] };
  ...
  const { data } = await supabase
    ...
    .eq('status', 'pending');
  if (!data || data.length === 0) return { count: 0, assignments: [], recruited: [] };

  const rows = (data as Record<string, unknown>[]).map(r => {
    const sameFaction = (r.faction as string) === playerFaction;
    const baseLoyalty = (r.loyalty as number) ?? 50;
    const loyalty = sameFaction ? Math.min(99, baseLoyalty + 10) : baseLoyalty;
    return {
      save_id: saveId,
      user_id: userId,
      ...
      position: '待分配',
      role: '新录用干部',
      avatar_id: r.avatar_id as number,
      ...
      sub_level: 1, // 新录用公务员从科员级(1)起步，无选调生
      is_appointed: false,
      appointed_role: null,
      appointed_dept: null,
      dept_position: 'deputy',
  ...
  // 标记已录用为 selected，其余 pending 全部 dismissed
  await supabase.from('recruit_candidates').update({ status: 'selected' }).in('id', selectedIds);
  ...
  const recruited = (data as Record<string, unknown>[]).map(rowToRecruit);
  return { count: rows.length, assignments, recruited };
}
...
export async function getOrCreateQuarterCandidates(
  ...
  const rows = Array.from({ length: count }, () => {
    ...
    // 候选人派系归属：从五大阵营中随机分配（用于同派系/冲突判定）
    const faction = ALL_FACTIONS[Math.floor(Math.random() * ALL_FACTIONS.length)];
...
export async function processNominations(
  ...
  for (const row of data as Record<string, unknown>[]) {
    ...
    const reqDays = sub.nominationPosition === 'head' ? 5 : sub.nominationPosition === 'deputy' ? 2 : 0;
    if (waitDays < reqDays) continue; // 未到期

    // 随机扰动（反映上级态度与派系博弈）
    const noise = Math.floor(Math.random() * 20) - 10;
    const pass = sub.nominationPosition === 'staff'
      ? true
      : (sub.ability + noise) >= 45 && sub.integrity >= 38 && sub.loyalty >= 28;

    if (pass && sub.nominationDept && sub.nominationPosition) {
      // 正式任命（科员无需能力门槛，即时通过）
      const roleLabel = sub.nominationPosition === 'head'
        ? getDeptHeadTitle(sub.nominationDept, rankLevel)
        : sub.nominationPosition === 'deputy'
          ? getDeptDeputyTitle(sub.nominationDept, rankLevel)
          : `${getDeptNameByRank(sub.nominationDept, rankLevel)}科员`;
...
// ════════════════════════════════════════════════════════════════
// R3 派遣任务 & 忠诚赏赐 & R4 门客/师徒/举荐
// ════════════════════════════════════════════════════════════════

/** 派遣失败率：max(2%, 10%−(ability−40)/5%) */
export function computeDispatchFailRate(ability: number): number {
  const raw = 10 - (ability - 40) / 5;
  return Math.min(10, Math.max(2, raw)) / 100;
}

/** 派遣下属执行任务 */
export async function dispatchSubordinate(
  saveId: string, subId: string, type: DispatchType, currentDay: number,
): Promise<{ success: boolean; msg: string }> {
  const { DISPATCH_CONFIG } = await import('@/types/game');
  const cfg = DISPATCH_CONFIG[type];
  const { data: sub } = await supabase.from('subordinates').select('*').eq('id', subId).maybeSingle();
  if (!sub) return { success: false, msg: '未找到该下属' };
  if (sub.is_dispatched) return { success: false, msg: '该下属正在执行其他任务，无法派遣' };
  const reqVal = cfg.req.value;
  const ok = cfg.req.kind === 'ability' ? (sub.ability as number) >= reqVal : (sub.loyalty as number) >= reqVal;
  if (!ok) return { success: false, msg: `${sub.name} 的${cfg.req.kind === 'ability' ? '能力' : '忠诚'}不达标（需≥${reqVal}）` };
  const { error } = await supabase.from('subordinates').update({
    is_dispatched: true, dispatch_type: type,
    dispatch_start_day: currentDay, dispatch_end_day: currentDay + cfg.days,
  }).eq('id', subId);
  if (error) return { success: false, msg: '派遣失败，请重试' };
  return { success: true, msg: `已派遣 ${sub.name} 执行「${cfg.label}」任务，预计 ${cfg.days} 天后归来` };
}

export interface DispatchSettleItem {
  subId: string; subName: string; type: DispatchType; success: boolean;
  merit: number; silver: number; contribution: number; influence: number; security: number; loyaltyDelta: number;
}

/** 结算到期的派遣任务，返回收益汇总（供前端展示与 P4 钩子） */
export async function settleDispatches(saveId: string, currentDay: number): Promise<DispatchSettleItem[]> {
  const { DISPATCH_CONFIG } = await import('@/types/game');
  const { data } = await supabase.from('subordinates').select('*')
    .eq('save_id', saveId).eq('is_dispatched', true).lte('dispatch_end_day', currentDay);
  if (!data || data.length === 0) return [];
  const results: DispatchSettleItem[] = [];
  for (const sub of data) {
    const type = sub.dispatch_type as DispatchType;
    const cfg = DISPATCH_CONFIG[type];
    if (!cfg) continue;
    const ability = sub.ability as number;
    const failRate = computeDispatchFailRate(ability);
    const isSuccess = Math.random() >= failRate;
    const item: DispatchSettleItem = {
      subId: sub.id as string, subName: sub.name as string, type, success: isSuccess,
      merit: 0, silver: 0, contribution: 0, influence: 0, security: 0, loyaltyDelta: 0,
    };
    if (isSuccess) {
      if (type === 'inspect') {
        item.merit = 15 + Math.floor(Math.random() * 16); // 15~30
        item.contribution = 10;
      } else if (type === 'invest') {
        item.silver = 150 + Math.floor(Math.random() * 151); // 150~300
        item.merit = 20;
      } else if (type === 'stabilize') {
        item.influence = 3 + Math.floor(Math.random() * 4); // 派系声望
        item.security = 2 + Math.floor(Math.random() * 4); // 治安
      } else if (type === 'special') {
        item.contribution = 30 + Math.floor(Math.random() * 21); // 30~50
        item.merit = 40;
      }
    } else {
      item.loyaltyDelta = -5; // 失败忠诚-5
    }
    results.push(item);
    // 清除派遣标记（成功/失败均清除）
    await supabase.from('subordinates').update({
      is_dispatched: false, dispatch_type: null, dispatch_start_day: null, dispatch_end_day: null,
      loyalty: Math.max(0, Math.min(100, (sub.loyalty as number) + item.loyaltyDelta)),
    }).eq('id', sub.id as string);
  }
  // 汇总收益写入玩家存档
  const totalMerit = results.reduce((s, r) => s + r.merit, 0);
  const totalSilver = results.reduce((s, r) => s + r.silver, 0);
  const totalContribution = results.reduce((s, r) => s + r.contribution, 0);
  const totalInfluence = results.reduce((s, r) => s + r.influence, 0);
  const totalSecurity = results.reduce((s, r) => s + r.security, 0);
  if (totalMerit || totalSilver || totalContribution || totalInfluence || totalSecurity) {
    const { data: save } = await supabase.from('player_saves').select('merit_points, silver, faction_contribution, faction_influence, security_index').eq('id', saveId).maybeSingle();
    if (save) {
      await supabase.from('player_saves').update({
        merit_points: Math.round((save.merit_points as number) + totalMerit),
        silver: Math.round((Number(save.silver) + totalSilver) * 100) / 100,
        faction_contribution: Math.round((save.faction_contribution as number) + totalContribution),
        faction_influence: Math.min(100, Math.round((save.faction_influence as number) + totalInfluence)),
        security_index: Math.min(100, Math.round((save.security_index as number) + totalSecurity)),
      }).eq('id', saveId);
    }
  }
  return results;
...
/** 赏赐下属：mode='silver' 消耗银两50 / mode='merit' 消耗政绩10 → loyalty +5~15 */
export async function rewardSubordinate(
  saveId: string, subId: string, mode: 'silver' | 'merit',
): Promise<{ success: boolean; msg: string }> {
  const { data: save } = await supabase.from('player_saves').select('silver, merit_points').eq('id', saveId).maybeSingle();
  if (!save) return { success: false, msg: '存档不存在' };
  if (mode === 'silver' && Number(save.silver) < 50) return { success: false, msg: '个人资金不足（需50）' };
  if (mode === 'merit' && (save.merit_points as number) < 10) return { success: false, msg: '政绩不足（需10）' };
  const { data: sub } = await supabase.from('subordinates').select('loyalty').eq('id', subId).maybeSingle();
  if (!sub) return { success: false, msg: '未找到该下属' };
  const gain = 5 + Math.floor(Math.random() * 11); // 5~15
  const newLoyalty = Math.min(100, Math.max(0, (sub.loyalty as number) + gain));
  await supabase.from('subordinates').update({ loyalty: newLoyalty }).eq('id', subId);
  await supabase.from('player_saves').update(
    mode === 'silver' ? { silver: Math.round((Number(save.silver) - 50) * 100) / 100 } : { merit_points: Math.round((save.merit_points as number) - 10) }
  ).eq('id', saveId);
  return { success: true, msg: `已赏赐 ${gain} 点忠诚度` };
}

/** 每日忠诚衰减：loyalty −0.05/天，<20 离职风险，<10 触发反水事件 */
export interface LoyaltyDecayResult { quitIds: string[]; betrayalIds: string[]; }
export async function dailyLoyaltyDecay(saveId: string, days: number): Promise<LoyaltyDecayResult> {
  const { data } = await supabase.from('subordinates').select('id, loyalty, is_dispatched').eq('save_id', saveId);
  if (!data || data.length === 0) return { quitIds: [], betrayalIds: [] };
  const quitIds: string[] = [];
  const betrayalIds: string[] = [];
  for (const sub of data) {
    const cur = sub.loyalty as number;
    const decayed = Math.max(0, Math.round((cur - 0.05 * days) * 100) / 100);
    if (decayed < 10) {
      betrayalIds.push(sub.id as string);
      // 触发反水事件（复用 eventType 框架）
      await supabase.from('subordinates').update({
        loyalty: decayed, event_type: 'betrayal', event_day: 0, event_handled: false,
      }).eq('id', sub.id as string);
    } else if (decayed < 20) {
      // 随机离职风险（约5%）
      if (Math.random() < 0.05) {
        quitIds.push(sub.id as string);
        await supabase.from('subordinates').delete().eq('id', sub.id as string);
      } else {
        await supabase.from('subordinates').update({ loyalty: decayed }).eq('id', sub.id as string);
      }
    } else {
      await supabase.from('subordinates').update({ loyalty: decayed }).eq('id', sub.id as string);
    }
  }
  return { quitIds, betrayalIds };
}

// ── R4 私属门客 ──────────────────────────────────────────────────
function rowToRetainer(row: Record<string, unknown>): Retainer {
  return {
    id: row.id as string,
    saveId: row.save_id as string,
    userId: row.user_id as string,
    type: row.type as RetainerType,
    name: row.name as string,
    avatarId: (row.avatar_id as number) ?? 0,
    ability: (row.ability as number) ?? 50,
    loyalty: Number(row.loyalty as number) ?? 60,
    bonus: (row.bonus as string) ?? '',
    createdAt: row.created_at as string,
  };
}

export async function getRetainers(saveId: string): Promise<Retainer[]> {
  const { data, error } = await supabase.from('retainers').select('*').eq('save_id', saveId).order('created_at', { ascending: true });
  if (error || !data) return [];
  return (data as Record<string, unknown>[]).map(rowToRetainer);
}

/** 招募私属门客：校验资源并扣除 */
export async function recruitRetainer(
  saveId: string, userId: string, type: RetainerType,
): Promise<{ success: boolean; msg: string }> {
  const { RETAINER_CONFIG } = await import('@/types/game');
  const cfg = RETAINER_CONFIG[type];
  const { data: save } = await supabase.from('player_saves').select('silver, connections').eq('id', saveId).maybeSingle();
  if (!save) return { success: false, msg: '存档不存在' };
  const silver = Number(save.silver);
  const connections = Number(save.connections);
  if (cfg.req.silver && silver < cfg.req.silver) return { success: false, msg: `个人资金不足（需${cfg.req.silver}）` };
  if (cfg.req.connections && connections < cfg.req.connections) return { success: false, msg: `人脉不足（需${cfg.req.connections}）` };
  await ensureNpcNamePoolLoaded();
  const name = pickNpcName();
  const { error } = await supabase.from('retainers').insert({
    save_id: saveId, user_id: userId, type, name,
    avatar_id: Math.floor(Math.random() * 8),
    ability: 50 + Math.floor(Math.random() * 31),
    loyalty: 50 + Math.floor(Math.random() * 31),
    bonus: cfg.effect,
  });
  if (error) return { success: false, msg: '招募失败，请重试' };
  await supabase.from('player_saves').update({
    silver: Math.round((silver - (cfg.req.silver ?? 0)) * 100) / 100,
    connections: Math.round((connections - (cfg.req.connections ?? 0)) * 100) / 100,
  }).eq('id', saveId);
  return { success: true, msg: `已招募${cfg.label}「${name}」，效果：${cfg.effect}` };
}

/** 赏赐门客：消耗银两50 → loyalty +5~15 */
export async function rewardRetainer(saveId: string, retainerId: string): Promise<{ success: boolean; msg: string }> {
  const { data: save } = await supabase.from('player_saves').select('silver').eq('id', saveId).maybeSingle();
  if (!save || Number(save.silver) < 50) return { success: false, msg: '个人资金不足（需50）' };
  const { data: r } = await supabase.from('retainers').select('loyalty').eq('id', retainerId).maybeSingle();
  if (!r) return { success: false, msg: '未找到该门客' };
  const gain = 5 + Math.floor(Math.random() * 11);
  await supabase.from('retainers').update({ loyalty: Math.min(100, Math.max(0, Number(r.loyalty) + gain)) }).eq('id', retainerId);
  await supabase.from('player_saves').update({ silver: Math.round((Number(save.silver) - 50) * 100) / 100 }).eq('id', saveId);
  return { success: true, msg: `已赏赐 ${gain} 点忠诚度` };
}

/** 门客每日忠诚衰减：loyalty −0.05/天，<20 流失，<10 叛离 */
export async function dailyRetainerDecay(saveId: string, days: number): Promise<{ lostIds: string[] }> {
  const { data } = await supabase.from('retainers').select('id, loyalty').eq('save_id', saveId);
  if (!data || data.length === 0) return { lostIds: [] };
  const lostIds: string[] = [];
  for (const r of data) {
    const cur = Number(r.loyalty);
    const decayed = Math.max(0, Math.round((cur - 0.05 * days) * 100) / 100);
    if (decayed < 10) {
      lostIds.push(r.id as string);
      await supabase.from('retainers').delete().eq('id', r.id as string);
    } else if (decayed < 20) {
      if (Math.random() < 0.05) {
        lostIds.push(r.id as string);
        await supabase.from('retainers').delete().eq('id', r.id as string);
      } else {
        await supabase.from('retainers').update({ loyalty: decayed }).eq('id', r.id as string);
      }
    } else {
      await supabase.from('retainers').update({ loyalty: decayed }).eq('id', r.id as string);
    }
...
// ── R4 师徒制 ────────────────────────────────────────────────────
/** 建立师徒关系：师傅 ability≥60，徒弟 ability<40 */
export async function setMentor(mentorId: string, apprenticeId: string): Promise<{ success: boolean; msg: string }> {
  const { data: mentor } = await supabase.from('subordinates').select('ability').eq('id', mentorId).maybeSingle();
  const { data: apprentice } = await supabase.from('subordinates').select('ability, mentor_id').eq('id', apprenticeId).maybeSingle();
  if (!mentor || !apprentice) return { success: false, msg: '未找到对应下属' };
  if ((mentor.ability as number) < 60) return { success: false, msg: '师傅能力不足（需≥60）' };
  if ((apprentice.ability as number) >= 40) return { success: false, msg: '徒弟能力过高（需<40）' };
  if (apprentice.mentor_id) return { success: false, msg: '该下属已有师傅' };
  await supabase.from('subordinates').update({ mentor_id: mentorId }).eq('id', apprenticeId);
  return { success: true, msg: '师徒关系已建立，徒弟能力成长 +20%' };
}

/** 师徒制效果：徒弟能力成长 +20%（供培养/考核结算调用） */
export function mentorGrowthMultiplier(apprentice: Subordinate): number {
  return apprentice.mentorId ? 1.2 : 1;
}

// ── R4 举荐入仕（呼应 P6，须过 年龄+编制+贡献 全套，不豁免）─────
export async function recommendIntoService(
  saveId: string, candidateId: string,
): Promise<{ success: boolean; msg: string }> {
  const { data: cand } = await supabase.from('recruit_candidates').select('*').eq('id', candidateId).maybeSingle();
  if (!cand) return { success: false, msg: '未找到候选人' };
  // 年龄校验：满18（呼应 P7）
  const birthYear = cand.birth_year as number | null;
  const { data: save } = await supabase.from('player_saves').select('faction_contribution, game_days, birth_year, rank_level, user_id').eq('id', saveId).maybeSingle();
  if (!save) return { success: false, msg: '存档不存在' };
  // 年龄校验：满18（呼应 P7）
  const currentYear = (save.birth_year as number) + Math.floor((save.game_days as number) / 365);
  if (birthYear && currentYear - birthYear < 18) return { success: false, msg: '候选人年龄不足18岁，无法举荐' };
  // 编制校验：未满145
  const { count } = await supabase.from('subordinates').select('*', { count: 'exact', head: true }).eq('save_id', saveId).is('transferred_city', null);
  if ((count ?? 0) >= SUBORDINATE_QUOTA_MAX) return { success: false, msg: '编制已满（145/145），无法举荐' };
  // 贡献校验
  const needContribution = 50;
  if ((save.faction_contribution as number) < needContribution) return { success: false, msg: `派系贡献不足（需${needContribution}）` };
  // 通过全部校验：扣除贡献，加入名册
  const rankLevel = save.rank_level as number;
  const sub = buildAppointedSub(saveId, save.user_id as string, 'organization', 'deputy', rankLevel);
  sub.name = cand.name as string;
  sub.ability = cand.ability as number;
  sub.loyalty = cand.loyalty as number;
  sub.integrity = cand.integrity as number;
  sub.experience = 0;
  sub.specialty = (cand.specialty as string) ?? sub.specialty;
  await supabase.from('subordinates').insert(sub);
  await supabase.from('player_saves').update({ faction_contribution: Math.round((save.faction_contribution as number) - needContribution) }).eq('id', saveId);
  await supabase.from('recruit_candidates').update({ status: 'selected' }).eq('id', candidateId);
  return { success: true, msg: `已举荐「${cand.name}」入仕，消耗 ${needContribution} 派系贡献` };
}
...
export async function getPlayerProfile(userId: string): Promise<PlayerProfile | null> {
  const { data } = await supabase
    ...
    .select('user_id, leaderboard_save_id, leaderboard_save_changed_at, region_code')
    .eq('user_id', userId)
    .maybeSingle();
  if (!data) return null;
  return {
    userId: data.user_id as string,
    leaderboardSaveId: (data.leaderboard_save_id as string) ?? null,
    leaderboardSaveChangedAt: new Date(data.leaderboard_save_changed_at as string).getTime(),
    regionCode: (data.region_code as string) ?? '',
  };
}

/** 首次注册：把当前主存档设为上榜存档（若无则创建 profile） */
export async function ensureLeaderboardSave(userId: string, saveId: string): Promise<void> {
  const profile = await getPlayerProfile(userId);
  if (profile?.leaderboardSaveId) return;
  await supabase.from('player_profiles').upsert({
    user_id: userId,
    leaderboard_save_id: saveId,
    leaderboard_save_changed_at: new Date().toISOString(),
    region_code: (await getSelectedDatabaseCode()) ?? '',
  }, { onConflict: 'user_id' });
}

/** 切换上榜存档（含5年周期冷却，由 Edge Function 校验） */
export async function setLeaderboardSave(saveId: string): Promise<{ success: boolean; error?: string; cooldownRemainingDays?: number }> {
  const { data, error } = await supabase.functions.invoke('set-leaderboard-save', {
    body: { saveId },
  });
  if (error) {
    const msg = data?.error ?? error.message;
    return { success: false, error: msg, cooldownRemainingDays: data?.cooldownRemainingDays };
  }
...
export async function getLeaderboard(params: {
  ...
  if (error || !data) {
    return { entries: [], myRank: null, myScore: 0, prevGap: null, total: 0 };
  }
  return data as LeaderboardResult;
}

/** 上报当日增量（原子累加，仅上榜存档生效） */
export async function recordDailyGain(saveId: string, metric: LeaderboardMetric, delta: number): Promise<void> {
  if (delta === 0) return;
  try {
    await supabase.rpc('record_daily_gain', { p_save_id: saveId, p_metric: metric, p_delta: delta });
  } catch {
    // 忽略上报异常，不影响主流程
  }
}
...
/** 领取最高榜里程碑奖励 */
export async function claimMilestone(milestone: LeaderboardMilestone): Promise<ClaimMilestoneResult & { error?: string; alreadyClaimed?: boolean }> {
  const { data, error } = await supabase.functions.invoke('claim-milestone', {
    body: { milestone },
  });
  if (error) {
    return { success: false, milestone, influence: 0, silver: 0, extra: null, error: data?.error ?? error.message, alreadyClaimed: data?.alreadyClaimed };
  }
  return data as ClaimMilestoneResult;
}
